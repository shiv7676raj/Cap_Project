...
TC-API-01 Account Type Field Matches UI Selection
    ${account}=    Get Account By Id    ${customer_id}      ${account_id}
...
TC-API-03 Source Account Balance Decreases By Transfer Amount
    ...
    Sleep    3s
    ${src_balance_after}=    Get Account Balance    ${SRC_ACCOUNT}
    ${expected}=    Evaluate    float(${SRC_BALANCE_BEFORE}) - float(${TRANSFER_AMT})
...
#    Click Element    ${LOGOUT}
    Perform Transfer For API Validation
    Sleep    3s
    ${dest_balance_after}=    Get Account Balance   ${DEST_ACCOUNT}
    ${expected}=    Evaluate    float(${DEST_BALANCE_BEFORE}) + float(${TRANSFER_AMT})
    Log To Console      Before=${DEST_BALANCE_BEFORE}
    Log To Console      After=${dest_balance_after}
    Should Be Equal As Numbers
    ...    ${dest_balance_after}
    ...    ${expected}
...
TC-API-05 GET Accounts Wrong Customer ID Returns Error
    ...
    ${response}=    Get Invalid Customer Accounts       999999999
